 ...
 * its own. So while a player stands in one with {@code soul_music} on, {@code MusicManagerMixin}
 * holds vanilla's music manager still and this plays a stream {@link SoulSynth} composes as it goes,
 * from the same blend and rank the sky and the bed read.
 *
...
public final class SoulMusicPlayer
{
    ...
    private static final ResourceLocation PLACEHOLDER = new ResourceLocation(SoulHome.MODID, "music/soul");
    ...
    private static boolean vanillaStopped;
    ...
    /**
     * Whether Minecraft's own music should hold off - read by {@code MusicManagerMixin} every tick.
     *
     * <p>True for the whole of a visit, rests between pieces included. The quiet between two pieces
     * is part of the soul's music, and vanilla's own track starting up in it would be exactly the
     * thing a player asked to be rid of.
     */
    ...
    /**
     * True once per visit, the first time it is asked: the tick on which whatever vanilla had
     * playing should be stopped. After that the mixin only holds the manager still, so vanilla's own
     * delay before its next track is not pushed back a hundred ticks every tick of a long visit.
     */
    public static boolean claimVanillaStop()
    {
        if (vanillaStopped)
        {
            return false;

 ...
 *
 * <p>Called from {@code SoulDimensionRenderInfo#renderSky}, NeoForge's hook for a dimension that
 * wants a sky of its own, with the view-rotation matrix vanilla draws its own sky with. Nothing here writes depth,
...
public final class SoulSkyRenderer
{
     ...
     */
    public static boolean render(int ticks, float partialTick, Matrix4f modelView, Matrix4f projection)
    {
        ...
        // vanilla's own sky does the same: a fresh stack seeded with the view rotation
    ...
    private static VertexBuffer buildStars()
    {
        ...
        for (int star = 0; star < 1_400; star++)
        {
            ...
            for (int corner = 0; corner < 4; corner++)
            {
                ...
                builder.addVertex((float) (px + out * sinYaw - along * cosYaw), (float) (py + lift),
                        (float) (pz + along * sinYaw + out * cosYaw));
            }
        }
    ...
    private static void vertex(BufferBuilder builder, Matrix4f pose, float elevation, float azimuth, float[] colour, float alpha)
    {
        ...
        builder.addVertex(pose, flat * (float) Math.sin(azimuth), (float) Math.sin(rise) * RADIUS, flat * (float) Math.cos(azimuth))

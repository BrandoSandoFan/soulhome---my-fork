...
public final class SuppressionRenderer
{
    ...
    private static final ResourceLocation SHADER = new ResourceLocation(SoulHome.MODID, "suppression");
    ...
    private static void drawAuras(RenderLevelStageEvent event)
    {
        ...
        final Matrix3f normal = pose.last().normal();
    ...
    private static void loop(
            VertexConsumer lines, Matrix4f matrix, Matrix3f normal, Vec3 centre, Vector3f left, Vector3f up,
        ...
        for (int i = 0; i <= SEGMENTS; i++)
        {
            ...
            if (previous != null)
            {
                ...
                lines.vertex(matrix, (float) previous.x, (float) previous.y, (float) previous.z)
                        .color(r, g, b, alpha)
                        .normal(normal, (float) d.x, (float) d.y, (float) d.z)
                        .endVertex();
                lines.vertex(matrix, (float) point.x, (float) point.y, (float) point.z)
                        .color(r, g, b, alpha)
                        .normal(normal, (float) d.x, (float) d.y, (float) d.z)
                        .endVertex();
            }

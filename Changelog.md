...
SoundSource is not extensible on Forge 47 - the finding, not a workaround
...
weather. Checked directly against the mapped Forge 47.3.0 jar rather than assumed: `SoundSource` is
a plain `final class`, not one of the enums Forge patches to be extensible, so there is no small
version of this to ship. Recorded in the class javadoc and the `sound_volume` config comment rather
than left as an open question - the mod's own volume knob under `AMBIENT` is the real, permanent
mix control on this line, not a placeholder for something better. `1.21.1` is its own separate
question, since NeoForge patches a different set of enums than Forge does.
...
The soul has sounds of its own

Every ambient sound a soulhome made was borrowed from somewhere else in the game, and half of them
were the sound of a block being placed or broken - moss, deepslate, bone, chain. In a dimension
whose entire purpose is placing blocks, that is not ambience: a player hearing deepslate go down
behind them turns round to see who did it. Most of the rest were cues that already mean something.
A portal hum says there is a portal nearby. A beacon hum is the sound the ascension ritual makes,
so the soul's own voice was playing the ritual at random, minutes apart, for no reason.

- Ten new sounds, one per voice, with three variants each, and nothing borrowed. Warm is embers
  rather than a campfire; cold is a chime with the block taken off it; a hollow soul is a low thud
  with air around it. A soul that is two things at once gets a sound built out of both - steam is
  the crackle opening the hiss, not a crackle and a chime played together.
- A resource pack can now replace the soul's warm sound without replacing every campfire in the
  game, which was impossible before.
- How far each sound carries is set per voice. A hollow room's thud reaches further than an ember
  does, because it is a bigger thing and not because it was thrown from further away.
- All of it is generated by a script in the repository (`tools/ambience`), so nothing here carries
  somebody else's licence and re-rendering it is how you check where it came from.
- Ported to `1.21.1`.

A soul you can hear when nothing is happening in it

There was no ambient bed, only the one-shots - a sound, then a minute or more of nothing. The
reason was a good one: the thing that makes ambient audio unbearable is repetition, and the surest
way not to write a loop a player can hum along with is not to have a loop. The danger is real. The
conclusion was not, because a loop nobody can notice is a solved problem.

- A quiet bed under the one-shots now, in the same Ambient/Environment category and under the same
  switch. Two long loops with no seam and nothing in them to hum along to: five bands of noise each
  drifting at its own slow rate, so no two seconds inside the loop are alike and the loop's own
  join is crossfaded away.
- How far your soul has climbed mixes it. A rank 0 soul is a small dry room; a rank V soul is a
  large airy one with the tail of the space in it. This is the one cue the fog cannot give you -
  you can hear how big your soul is without looking up.
- Quieter than the one-shots, which were already quieter than a block being placed. It fades in
  when you arrive and away when you leave, and a rank landing mid-build slides the room open over
  several seconds rather than switching it.
- It steps aside for the ritual, an ability and the key like everything else does, and comes back
  without a swell.
- It survives a resource pack reload, which the sounds you leave running in Minecraft otherwise do
  not.
